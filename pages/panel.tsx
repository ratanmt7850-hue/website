...
const Panel: React.FC = () => {
  ...
  const setData = (resdata: Result[]) => {
    ...
    for (const result of resdata) {
      ...
      const res1 = `${replaceDashWithStarThree(
        result.open_panna
      )} - ${replaceDashWithStar(result.open_digit)}`;
  ...
  const getCss = (vals: string) => {
    ...
    const s = n === 1 || n === 5;
    return (
      <div className="black position">
        <p>
          {part[0][0]} <br /> {part[0][1]} <br /> {part[0][2]}
        </p>
        <p className="Wet">{part[1]}</p>
        <p>
          {part[2][0]} <br /> {part[2][1]} <br /> {part[2][2]}
        </p>
      </div>
    );
  };
